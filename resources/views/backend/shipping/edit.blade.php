...
<div class="card">
  <h4><span class="badge badge-warning right"><b><i class="nav-icon fas fa-address-card"></i></i> CHỈNH SỬA GIAO HÀNG</b></span></h4>
  <div class="card-body">
      <form method="post" action="{{route('shipping.update',$shipping->id)}}">
        @csrf 
        @method('PATCH')
        <div class="form-group">
          <label for="inputTitle" class="col-form-label">Tên giao hàng <span class="text-danger">*</span></label>
        <input id="inputTitle" type="text" name="type" placeholder="Nhập tên"  value="{{$shipping->type}}" class="form-control">
        @error('title')
