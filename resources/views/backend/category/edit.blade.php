...
<div class="card">
  <h4><span class="badge badge-warning right"><b><i class="nav-icon fas fa-address-card"></i></i> CHỈNH SỬA DANH MỤC</b></span></h4>
  <div class="card-body">
      <form method="post" action="{{route('category.update',$category->id)}}">
        ...
        <div class="form-group">
          ...
          <input id="inputTitle" type="text" name="title" placeholder="Nhập tên danh mục..."  value="{{$category->title}}" class="form-control">
          @error('title')
        ...
        <div class="form-group">
          <label for="is_parent">Loại danh mục</label><br>
          <input type="checkbox" name='is_parent' id='is_parent' value='{{$category->is_parent}}' {{(($category->is_parent==1)? 'checked' : '')}}> Cha                        
        </div>
        {{-- {{$parent_cats}} --}}
        {{-- {{$category}} --}}
