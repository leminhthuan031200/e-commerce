...
<a class="btn btn-icon btn-transparent-dark dropdown-toggle" id="messagesDropdown" href="javascript:void(0);" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
  <i class="fas fa-envelope fa-fw"></i>
  <!-- Counter - Messages -->
  @if(count(Helper::messageList())>5)
      <span data-count="5" class="badge badge-danger badge-counter">5+</span>
 ...
 <div class="dropdown-menu dropdown-menu-end border-0 shadow animated--fade-in-up" id="message-items" aria-labelledby="messagesDropdown">
     ...
     @foreach(Helper::messageList() as $message)
                <a class="dropdown-item d-flex align-items-center" href="{{route('message.show',$message->id)}}">
                    <div class="dropdown-list-image mr-3">
                        @if($message->photo)
                        <img class="rounded-circle"width="45px" width="10" src="{{$message->photo}}" alt="profile">
                        @else 
                        <img class="rounded-circle" width="45px" src="{{asset('backend/img/avatar.png')}}" alt="default img">
                        @endif
                    </div>
                    <div class="font-weight-bold">
                        <div class="text-truncate"> {{$message->subject}}</div>
                ...
                @endif
            @endforeach
            <a class="dropdown-item text-center small text-gray-500" href="{{route('message.index')}}">Tất cả thư</a>
